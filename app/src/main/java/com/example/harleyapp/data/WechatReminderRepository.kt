 ...
 */
class WechatReminderRepository(context: Context) {
     ...
     *
     * @param settings 页面提交的新配置，间隔会被限制在5到1440分钟。
     *
    ...
    fun syncActiveNotifications(
        ...
        val editor = preferences.edit()
            .putStringSet(KEY_PENDING_NOTIFICATION_KEYS, notificationKeys.toSet())
        if (notificationKeys.isNotEmpty()) {
            editor.putLong(
                KEY_LAST_MESSAGE_AT,
                latestMessageAtMillis.coerceAtLeast(0L)
            )
        }
     ...
     */
    @Synchronized
    fun recordReminderShown(shownAtMillis: Long): Boolean {
        val success = preferences.edit()
